...
set "MAIN_MODULE=got_script_generator.main_modern"
set "ENHANCED_MODULE=got_script_generator.modern_example_usage"
...
if "%~1"=="--enhanced" (
    ...
    set "MAIN_MODULE=%ENHANCED_MODULE%"
    shift
    goto :parse_args
)
if "%~1"=="--cpu" (
...
echo    Module: %YELLOW%!MAIN_MODULE!%NC%
echo.
...
REM Ensure project package is importable
...
if "!MODE!"=="train" (
    ...
    python -c "import importlib, sys; importlib.import_module('!MAIN_MODULE!')" || (
        echo %RED%❌ Module not found: !MAIN_MODULE!%NC%
        pause
        exit /b 1
    )
    ...
    python -c "from got_script_generator.improved_helperAI import analyze_dataset; import json; result = analyze_dataset('!DATA_PATH!'); print(json.dumps(result, indent=2))"
