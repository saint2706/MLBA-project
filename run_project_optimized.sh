...
ENHANCED_SCRIPT="modern_example_usage.py"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --enhanced) MAIN_SCRIPT="$ENHANCED_SCRIPT"; shift 1 ;;
...
echo -e "   Script: ${YELLOW}$MAIN_SCRIPT${NC}"
echo ""
...
case $MODE in
    "train")
        ...
        if [ ! -f "$MAIN_SCRIPT" ]; then
            echo -e "${RED}❌ Script not found: $MAIN_SCRIPT${NC}"
            exit 1
        fi
...
"
    ...
    "analyze")
        ...
        python -c "
from improved_helperAI import analyze_dataset
import json
